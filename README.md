...
**Le lancer a deux boutons « Lancer », pas un seul déplacé.** Sur un écran court, la roue et le bouton de la barre d'actions ne tenaient pas ensemble à l'écran ; un second bouton, rond, posé au centre du disque — frère du rotor, pas son enfant, donc il ne tourne pas avec lui —, garantit que la roue reste visible au moment de viser. La barre garde le sien pour rester sous le pouce en bas d'écran, là où on s'attend à trouver un bouton d'action. Les deux commandent la même action, mais pas de la même façon quand le lancer est illégal : celui de la barre s'estompe (`aria-disabled`), celui du centre disparaît du DOM — au même instant, sur la même valeur de légalité. Un bouton invisible mais focalisable serait pire qu'un bouton absent, et la barre garde le sien pour porter l'accessibilité clavier du lancer — ce qui n'est vrai que parce que le bouton central est `tabIndex={-1}` : la roue étant montée avant la grille, les scores et la barre, il serait sinon le premier des deux atteint à la tabulation, et le focus retomberait sur `<body>` dès le lancer parti, puisqu'il se démonte. Il reste cliquable et annoncé, seulement hors du chemin de tabulation. Leur texte visible est identique (« Lancer », « Stop », « Tourner » selon le mode), mais leur nom accessible diffère — le bouton central ajoute « au centre de la roue » — pour qu'un lecteur d'écran ou un test par rôle et nom puisse encore les distinguer ; deux contrôles portant le même nom accessible seraient indiscernables l'un de l'autre.
...
1020 tests sur 46 fichiers. Le moteur est couvert par des tests unitaires, un scénario de partie scripté et un fuzz d'invariants — c'est la partie du code où une régression est invisible à l'écran.
